# Phase 1 & 2A: DATA CLEANING, PREP & EDA I
- Consolidated datasets from features, sales, and stores
...
- Created Month, Month_Name, and Week columns using lubridate.
- Checked and handled NA values.
- Saved clean dataset as smartsell.
...
smartsell <- merge(sales_features, stores, by = "Store", all.x = TRUE)
...
smartsell$Type <- as.factor(smartsell$Type)
smartsell$IsHoliday.x <- as.factor(smartsell$IsHoliday.x)
smartsell$IsHoliday.y <- as.factor(smartsell$IsHoliday.y)
...
plot2 <- ggplot(smartsell, aes(x = Type, y = Weekly_Sales, fill = Type)) +
  geom_boxplot() +
  scale_y_continuous(labels = dollar) +
